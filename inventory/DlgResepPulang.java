...
public final class DlgResepPulang extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        CopyResep = new javax.swing.JMenuItem();
        ...
        CopyResep.setFont(new java.awt.Font("Tahoma", 0, 11)); // NOI18N
        CopyResep.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png"))); // NOI18N
        CopyResep.setText("Cetak Resep");
...
private void ChkInputActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ChkInputActionPerformed
  isForm();                
    ...
    private void CopyResepActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_CopyResepActionPerformed
            ...
            param.put("peresep",Sequel.cariIsi("select nm_dokter from dokter where kd_dokter=?",Sequel.cariIsi("select kd_dokter from dpjp_ranap where no_rawat=?",TNoRw.getText())));
            ...
            param.put("umur",Sequel.cariIsi("select umur from pasien where no_rkm_medis=?",Sequel.cariIsi("select no_rkm_medis from reg_periksa where no_rawat=?",TNoRw.getText())));
            param.put("jk",Sequel.cariIsi("select jk from pasien where no_rkm_medis=?",Sequel.cariIsi("select no_rkm_medis from reg_periksa where no_rawat=?",TNoRw.getText())));
            param.put("nik",Sequel.cariIsi("select no_ktp from pasien where no_rkm_medis=?",Sequel.cariIsi("select no_rkm_medis from reg_periksa where no_rawat=?",TNoRw.getText())));
            param.put("no_telp",Sequel.cariIsi("select no_tlp from pasien where no_rkm_medis=?",Sequel.cariIsi("select no_rkm_medis from reg_periksa where no_rawat=?",TNoRw.getText())));
            param.put("no_peserta",Sequel.cariIsi("select no_peserta from pasien where no_rkm_medis=?",Sequel.cariIsi("select no_rkm_medis from reg_periksa where no_rawat=?",TNoRw.getText())));
            ...
            param.put("alamat",Sequel.cariIsi("select alamat from pasien where no_rkm_medis=?",Sequel.cariIsi("select no_rkm_medis from reg_periksa where no_rawat=?",TNoRw.getText())));
            Valid.MyReport("rptLembarResep.jasper",param,"::[ Resep ]::");
            this.setCursor(Cursor.getDefaultCursor());
        }
    ...
    */
    ...
    private javax.swing.JMenuItem CopyResep;
